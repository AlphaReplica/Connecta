 ...
 */
var Connecta = (function(url,reconnect,stunUrls)
{
    var scope = this;
    ...
    function init()
    {
        scope._servConn = new ConnectaSocket(scope._url,scope._reconnect);
    ...
    function onClientJoinedRoom(id)
    {
        if(scope._servConn.useRTC)
        {
            ...
            {
                getOrCreatePeer(id).createOffer();
            }
            else
            {
    ...
    {
        scope.dispatchEvent("peerMessage",{message:data,id:from,fallback:true});
    }
