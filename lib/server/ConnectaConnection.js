...
{
    console.warn('UWS Module not found!');
}

if(WebSocket == null)
{
    console.log('Using WS Module!');
    
    WebSocket = require('ws');
}
else
{
    console.log('Using UWS Module!');
}
...
class ConnectaConnection
{
    ...
    */
    constructor(port,params,autoStart)
    {
        ...
        if(params.pingInSeconds>0)
        {
            this.pingInterval = setInterval(this.pingClients.bind(this),params.pingInSeconds * 1000);
        }
    }

    /**
     * Pings Clients by setting waitingForPingResponse flag to true and sending ping event
     * @method pingClients
    */
    pingClients()
    {
        ...
        {
            if(this.peers[key].waitingForPingResponse == true)
            {
                this.peers[key].close();
            }
            else
            {
                this.peers[key].waitingForPingResponse = true;
                this.sendEventToPeer(key,InternalEvents.PING);
            }
        }
    }
    ...
    */
    onDisconnect(client)
    {
        if(this.onBeforeClientDisconnected)
        {
            this.onBeforeClientDisconnected(client);
        }
        let id = client.id;
        if(this.peers[id])
        {
    ...
    onStringMessage(msg,client)
    {
        if(msg.length>0)
        {
            ...
            switch(char)
            {
                ...
                {
                    client.waitingForPingResponse = false;
                    break;
                }
